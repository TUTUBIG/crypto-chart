 ...
 */
export function deserializeCandleData(data: ArrayBuffer): Candle[] {
  console.log("deserializeCandleData", data.byteLength);
  ...
  for (let i = 0; i < count; i++) {
    ...
    const volumeIn = view.getFloat64(offset, true);
    offset += 8;

    // Read VolumeOut (8 bytes, little-endian float64)
    const volumeOut = view.getFloat64(offset, true);
    offset += 8;
 ...
 */
export function deserializeRealTimeTradeData(data: ArrayBuffer): RealTimeTrade {
  ...
  try {
    ...
    // Read AmountIn (8 bytes, little-endian float64)
    const amountIn = view.getFloat64(offset, true);
    console.log('AmountIn:', amountIn);
    offset += 8;

    // Read AmountOut (8 bytes, little-endian float64)
    const amountOut = view.getFloat64(offset, true);
